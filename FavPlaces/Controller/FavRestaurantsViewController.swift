//
//  FavRestaurantsViewController.swift
//  Foodster
//
//  Created by Magzhan Zhumaly on 25.12.2022.
//

import UIKit
import CoreData

class FavRestaurantsViewController: UIViewController {
...
//        cityButton.image = UIImage(named: "AppIcon")
        ...
        var uiActions = [UIAction(title: citiesDict[0], state: .on, handler: artTagOptionClosure)]
        
        for i in 1..<citiesDict.count {
            uiActions.append(UIAction(title: citiesDict[i], handler: artTagOptionClosure))
        }
...
//                    if self.cityButton.title == restaurantLocal.city && restaurantLocal.isFavorite {
//
...
//            fetchRequest.predicate = NSPredicate(format: "name CONTAINS[c] %@ OR location CONTAINS[c] %@ AND isFavorite == %@", searchText, searchText, NSNumber(value: true))
            fetchRequest.predicate = NSPredicate(format: "name CONTAINS[c]  %@ AND isFavorite == %@" , searchText, NSNumber(value: true))
...
extension FavRestaurantsViewController: UITableViewDelegate {
...
extension FavRestaurantsViewController: NSFetchedResultsControllerDelegate {
...
extension FavRestaurantsViewController: UISearchResultsUpdating {
